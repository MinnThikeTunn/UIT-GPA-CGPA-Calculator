---
name: pencil-design
description: Create professional visual designs from natural language descriptions using the Pencil CLI. Use this skill whenever the user wants to create visual designs, mockups, UI mockups, landing pages, app screens, graphics, images from text descriptions, or any design generation task. Also use when user mentions design tools, visual creation, creating mockups, or wants to generate any visual output from a text prompt. The Pencil CLI generates .pen files (structured JSON design format) and exports them as PNG/JPEG/WebP/PDF images.
---
